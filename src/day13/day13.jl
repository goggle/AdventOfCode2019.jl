...
function day13(input::String = readInput(joinpath(@__DIR__, "input.txt")))
    ...
    output = AdventOfCode2019.Day05._run_program(copy(program), nothing, nothing, nothing)
...
function _play_game(program::Array{Int,1})
    ...
    t = @task AdventOfCode2019.Day05._run_program(copy(program), input, output, nothing)
    schedule(t)
